...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', 
                    handlers=[
                        logging.FileHandler("fetch_log_3min.txt"),
...
def fetch_and_save_data(symbol):
    filename = f"{DATA_DIR}/{symbol}_3minute.csv"
    ...
    while current_start < end_date:
        ...
        try:
            ...
            data = kite.historical_data(token, current_start, current_end, "3minute")
            if data:
